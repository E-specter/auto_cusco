...
- 2026-09-14: entregados B4, B5 y B6b (migración `7a91278726ae`, sin commit). Lo implementado y la medición de 46 000 filas están en `docs/mowa-mes.md` §9 a §14. `tests/test_codigos_mowa_mes.py` queda en rojo hasta que dev_frontend agregue las 12 claves `mowaMes.codigo.*`. Siguiente: B7 (escenarios Playwright con `e2e/api-falsa-mowa-mes.ts`, acordados con dev_frontend). La base local es compartida: toda prueba o script que cree campañas debe restituir supervisores, configuración y `usada_en` del Speech original.
